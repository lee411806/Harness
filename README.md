...
> **현재 단계: 가이드 + 부분 제어 (하네스 시작)**
>
> - 가이드 = "이렇게 행동해" → 소크라테스 리뷰 방식
> - 제어 = "이건 못 하게 막아" → 클래스 단위 코드 작성 강제
> - 추가 제어(Permissions, Hooks)는 필요할 때 추가한다.
...
| 상황 | 규칙 |
|------|------|
| **신규 생성** | 클래스 하나씩 작성 → 확인 → 다음 클래스 |
| **수정/리팩토링** | 여러 파일 동시 수정 가능 |
